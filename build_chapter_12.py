"""Build Chapter 12 notebook: Monitoring and Scaling."""
import sys, pathlib
...
cells.append(md(
    ...
    "\n"
    "We build the data-drift detectors, monitoring metrics, scaling-strategy spreadsheet "
    "calculations, fairness audit utility, and retraining-trigger logic the chapter describes. "
    "Each section produces a small, self-contained artefact that can be wired into a real "
    "system.\n"
))
...
cells.append(md("## 2. KS test and PSI for feature-drift detection\n"
                "\n"
                "The KS test compares cumulative distribution functions; PSI bins values and "
                "computes a weighted log-ratio."))
cells.append(code(
...
cells.append(md("## 3. CUSUM chart for detecting sudden changes\n"
                "\n"
                "PSI and KS compare the whole-window distribution; CUSUM is sensitive to a sudden "
                "change in the mean."))
cells.append(code(
...
cells.append(md("## 4. Fairness audit - equalized odds\n"
                "\n"
                "We compute true positive rate (TPR) and false positive rate (FPR) per group and "
                "flag when the gap exceeds a configurable threshold."))
cells.append(code(
...
cells.append(md("## 5. Audit record with content hash\n"
                "\n"
                "The sha256 hash binds the metrics record to the exact model + dataset combo so "
                "auditors can verify no tampering happened later."))
cells.append(code(
...
cells.append(md("## 6. Triggered-vs-scheduled retraining decision logic"))
cells.append(code(
...
cells.append(md("## 7. Scaling cost estimation\n"
                "\n"
                "Simple spreadsheet calculation: given peak/trough RPS, target latency, instance "
                "throughput and instance price, compute monthly cost for three strategies."))
cells.append(code(
...
cells.append(md("## 8. Putting it together - a monitoring dashboard sketch"))
cells.append(code(
...
cells.append(md("### 9.2 12% confidence drop on day 45\n"
...
cells.append(md("### 9.3 Fraud-detection monitoring strategy\n"
...
cells.append(md("### 9.4 KS vs PSI vs CUSUM\n"
...
cells.append(md("### 9.5 KEDA scale-out trace\n"
...
cells.append(md("### 9.6 PPO PSI 0.35 without return degradation\n"
...
cells.append(md("### 9.7 Stratified sampling for TPR gap\n"
...
cells.append(md("### 9.8 Three cost strategies for 800 RPS peak / 50 RPS trough\n"
