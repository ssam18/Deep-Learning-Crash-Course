"""Build Chapter 10 notebook: Reinforcement Learning."""
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).parent))
from _nbutils import build_notebook, md, code

CHAPTER = 10
OUT = pathlib.Path(__file__).parent / f"Chapter_{CHAPTER:02d}" / f"Chapter_{CHAPTER:02d}_Reinforcement_Learning.ipynb"
...
cells.append(md(
    ...
    "\n"
    "We build a tabular Q-learning agent on FrozenLake, a Deep Q-Network on CartPole-v1, and a "
    "minimal Proximal Policy Optimization (PPO) implementation. Each algorithm is wrapped so it "
...
cells.append(md("## 2. Tabular Q-learning on FrozenLake\n"
                "\n"
                "Classical Q-learning with an epsilon-greedy policy. We use the deterministic "
                "(non-slippery) 4x4 FrozenLake from Gymnasium."))
cells.append(code(
    "try:\n"
...
cells.append(md("## 3. Deep Q-Network on CartPole-v1\n"
                "\n"
                "Two stabilising tricks: replay buffer and target network. The training loop is "
                "deliberately short to keep CPU runtime under a minute."))
cells.append(code(
    "try:\n"
    "    import gymnasium as gym\n"
    "    import torch\n"
...
cells.append(md("## 4. REINFORCE with a baseline (vanilla policy gradient)\n"
                "\n"
                "Direct optimisation of $\\theta$ to maximise the expected return. A learnt value "
...
cells.append(md("## 5. PPO from scratch (clipped surrogate objective)\n"
                "\n"
                "A minimal PPO loop with GAE-$\\lambda$ advantages. With the default hyperparameters "
                "below, the agent solves CartPole (reward > 475) in under one minute on CPU."))
cells.append(code(
    "try:\n"
    "    import gymnasium as gym\n"
    "    import torch\n"
...
cells.append(md("## 6. Exercise solutions\n"
                "\n"
                "### 6.1 MCQ answer key\n"
                "\n"
                "| Q | Answer | Why |\n"
...
cells.append(md("### 6.2 Bellman optimality derivation\n"
                ...
                "\n"
                "The max operator makes Q-learning **off-policy**: the update uses the *greedy* action of "
                "the next state regardless of what action the behaviour policy actually selected, so we "
                "can learn the optimal policy while behaving sub-optimally for exploration."))

cells.append(md("### 6.3 DQN failing on a sparse-reward video game\n"
                "Three interventions:\n"
...
cells.append(md("### 6.4 DQN vs PPO stability\n"
...
cells.append(md("### 6.5 Variance reduction with a baseline\n"
                "The policy gradient is $\\nabla \\log \\pi(a|s) (G_t - b(s))$. For any baseline that does not "
                "depend on the action,\n"
                "$\\mathbb{E}_a[\\nabla \\log \\pi(a|s) b(s)] = b(s) \\nabla \\sum_a \\pi(a|s) = b(s) \\nabla 1 = 0$,\n"
                "so subtracting $b(s)$ does not change the expected gradient. Variance is reduced because "
                "the magnitude of $G_t - b(s)$ is typically much smaller than $G_t$, especially when "
                "$b(s) \\approx V(s)$. This is exactly what the value-function baseline does in REINFORCE-with-baseline."))

cells.append(md("### 6.6 Why multiple gradient epochs work in PPO\n"
                "PPO's clipped surrogate $\\min(r_t A_t, \\text{clip}(r_t, 1-\\epsilon, 1+\\epsilon) A_t)$ "
                "limits how far the new policy can drift from the old one **on each parameter update**. "
                "Each epoch becomes a small, bounded trust-region step. Without clipping, the importance "
                "ratio $r_t = \\pi_\\theta(a|s) / \\pi_{\\theta_{\\text{old}}}(a|s)$ can grow large after a few "
                "gradient steps, the off-policy correction becomes very noisy, and naive policy "
                "gradient diverges. The clipping is what makes data reuse safe."))

cells.append(md("### 6.7 RL system for mobile-ad placement\n"
                "**MDP formulation:**\n"
...
cells.append(md("### 6.8 Full RLHF pipeline\n"
