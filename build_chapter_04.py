"""Build Chapter 4 notebook: Convolutional Neural Networks for image classification."""
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).parent))
from _nbutils import build_notebook, md, code

CHAPTER = 4
OUT = pathlib.Path(__file__).parent / f"Chapter_{CHAPTER:02d}" / f"Chapter_{CHAPTER:02d}_Convolutional_Neural_Networks.ipynb"
...
cells.append(md(
    ...
    "\n"
    "This notebook implements the chapter's convolutional models on CIFAR-10, demonstrates "
    "residual connections and batch normalisation, plots feature maps and Grad-CAM, and "
    "concludes with the chapter exercises.\n"
))
...
cells.append(md("## 2. Convolution from scratch in NumPy\n"
                "\n"
...
cells.append(md("## 3. Output dimensions, stride, padding\n"
                "\n"
                "The output spatial dimension is $(W - K + 2P) / S + 1$. The cell verifies that "
                "formula for many configurations."))
cells.append(code(
...
cells.append(md("## 4. CIFAR-10 baseline CNN in Keras"))
cells.append(code(
...
cells.append(md("## 5. Residual block + BatchNorm + augmentation"))
cells.append(code(
...
cells.append(md("## 6. Visualising what the network learns: feature maps + Grad-CAM"))
cells.append(code(
...
cells.append(md("## 7. Transfer learning preview - feature extraction with MobileNetV2\n"
                "\n"
                "Full transfer learning is the subject of Chapter 8, but freezing a pretrained "
                "backbone and adding a small head is a one-liner in Keras."))
cells.append(code(
...
cells.append(md("## 8. Exercise solutions\n"
                "\n"
                "### 8.1 MCQ answer key\n"
                "\n"
                "| Q | Answer | Why |\n"
...
cells.append(md("### 8.2 Why convolutions beat fully connected layers on images\n"
                "Three reasons:\n"
...
cells.append(md("### 8.3 Output-size arithmetic\n"
                "Already verified in section 3. The general formula is\n"
                "$$o = \\lfloor (i - k + 2p) / s \\rfloor + 1$$"))

cells.append(md("### 8.4 Residual connections and the degradation problem\n"
                "Plain stacks of conv layers showed surprisingly *higher training error* as depth grew "
                "past 20 layers - the network had trouble even fitting the training set. He et al. "
                "showed that learning a residual $F(x)$ on top of the identity $x$ is much easier "
                "than learning $H(x)$ from scratch: if the optimal mapping is close to the identity, "
                "$F(x) \\approx 0$ is reachable, but learning $H(x) \\approx x$ in a stack of nonlinear "
                "layers requires precise weight cooperation."))

cells.append(md("### 8.5 BatchNorm placement\n"
                "The canonical order is **Conv -> BatchNorm -> Activation**. BatchNorm before the "
                "non-linearity ensures the input to the non-linearity has roughly zero mean and unit "
                "variance, which keeps gradients in the well-behaved part of ReLU. The "
                "BatchNorm-after-ReLU ordering exists in some papers but is less common."))

cells.append(md("### 8.6 Data augmentation for a traffic-sign classifier\n"
...
cells.append(md("### 8.7 Transfer learning - what is frozen?\n"
                "In feature-extraction mode every backbone weight is frozen (`trainable=False` in Keras, "
                "`requires_grad=False` in PyTorch). Only the new classification head learns. BatchNorm "
                "running statistics must also stay in inference mode - that is what `training=False` "
                "in the Keras call achieves."))

cells.append(md("### 8.8 Lower learning rate during fine-tuning\n"
                "Pretrained backbone weights already represent useful features. A large LR will move "
                "them far away in the first few steps before the head has converged, destroying the "
                "very features that made transfer useful. Typical recipe: head LR $10^{-3}$, "
                "backbone LR $10^{-5}$ once unfrozen."))

cells.append(md("### 8.9 Global Average Pooling vs Flatten + Dense\n"
                "GAP reduces a $(H, W, C)$ feature map to a $C$-vector by averaging over space. "
                "It eliminates the huge dense layer that Flatten otherwise feeds, sharply reducing parameter count and over-fitting risk, and it is permutation-invariant in space."))

cells.append(md("### 8.10 Grad-CAM\n"
                "Grad-CAM highlights the **class-discriminative spatial regions** the CNN attended to. "
                "It computes the gradient of the target class score with respect to the last convolutional feature map, averages those gradients per channel to obtain weights, and combines them into a heatmap. Section 6 above shows it in action."))

cells.append(md("### 8.11 EfficientNet compound scaling\n"
                "EfficientNet's scaling coefficients $\\phi$ multiply depth, width and resolution simultaneously: "
                "$\\text{depth} = \\alpha^\\phi, \\text{width} = \\beta^\\phi, \\text{resolution} = \\gamma^\\phi$, "
                "subject to $\\alpha \\beta^2 \\gamma^2 \\approx 2$. Scaling all three together gives better "
                "accuracy at a fixed FLOPs budget than scaling any one dimension alone."))
