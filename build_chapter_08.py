"""Build Chapter 8 notebook: Transfer Learning."""
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).parent))
from _nbutils import build_notebook, md, code

CHAPTER = 8
OUT = pathlib.Path(__file__).parent / f"Chapter_{CHAPTER:02d}" / f"Chapter_{CHAPTER:02d}_Transfer_Learning.ipynb"
...
cells.append(md(
    ...
    "\n"
    "We use MobileNetV2 / ResNet50 as feature extractors on CIFAR-10, demonstrate two-phase "
    "fine-tuning with discriminative learning rates, apply MixUp and RandAugment, show LoRA on "
    "a small text classifier, and finish with the chapter's exercises.\n"
))
...
cells.append(md("## 2. Feature extraction with MobileNetV2 (Keras)\n"
                "\n"
                "We freeze the backbone, attach a small classification head, and train only the "
                "head. CIFAR-10 is upsampled to 96x96 to match the pretrained input size."))
cells.append(code(
...
cells.append(md("## 3. Two-phase fine-tuning\n"
                "\n"
...
cells.append(md("## 4. Discriminative learning rates in PyTorch\n"
                "\n"
                "Assign exponentially decaying LRs from the head back to the early layers. The "
                "head LR is ~$10^{-3}$, the first conv block ~$10^{-6}$."))
cells.append(code(
    "try:\n"
    "    import torch\n"
...
cells.append(md("## 5. MixUp augmentation\n"
                "\n"
                "MixUp blends image *and label* pairs, regularising the model toward linear "
                "interpolations of the training set."))
cells.append(code(
...
cells.append(md("## 6. LoRA-style low-rank adapters\n"
                "\n"
                "LoRA freezes the pretrained weights and adds a small low-rank update "
                "$\\Delta W = A B$ where $A \\in \\mathbb{R}^{d \\times r}$ and $B \\in \\mathbb{R}^{r \\times d}$ "
                "with $r \\ll d$. Below is a from-scratch demonstration on a single nn.Linear; the "
                "PEFT library applies the same idea to every attention layer."))
cells.append(code(
    "try:\n"
    "    import torch\n"
    "    import torch.nn as nn\n"
...
cells.append(md("## 7. Maximum Mean Discrepancy (MMD) for domain adaptation\n"
                "\n"
                "We compute a Gaussian-kernel MMD between two distributions. Adding it to the loss "
                "encourages the feature distributions of source and target domains to align."))
cells.append(code(
    "import torch\n"
...
cells.append(md("### 8.2 Coding exercise 1 - Feature extraction baseline (sketch)\n"
                "Already implemented in section 2. Replace `MobileNetV2(input_shape=(96, 96, 3))` "
                "with `MobileNetV3Large(input_shape=(224, 224, 3))` to match the chapter's spec, "
                "and add `tf.keras.layers.Resizing(224, 224)` accordingly. Expected test accuracy "
                "after 10 epochs of head-only training is **>85 percent** on CIFAR-10 with the "
                "default Adam learning rate."))

cells.append(md("### 8.3 Coding exercise 2 - Two-phase fine-tuning\n"
                "Section 3 implements two-phase fine-tuning on a 30-layer unfreeze; for the "
                "10-layer / 10-epoch / $10^{-5}$ variant in the exercise simply change "
                "`backbone.layers[:-10]` and rerun. Typical accuracy gain over feature extraction "
                "alone on CIFAR-10: **2-4 percentage points**."))

cells.append(md("### 8.4 Coding exercise 3 - Discriminative LRs on Oxford-IIIT Pet\n"
                "Use the parameter grouping in section 4 with LRs $[10^{-3}, 3 \\times 10^{-4}, 10^{-4}, 3 \\times 10^{-5}, 10^{-5}, 3 \\times 10^{-6}, 10^{-6}]$ applied to "
                "the head + ResNet50 layer4..layer1 + stem. Combine with a cosine schedule over "
                "10 epochs; you should reach ~92-93 percent on the 37-class Pet dataset on a single "
                "GPU."))

cells.append(md("### 8.5 Coding exercise 4 - DistilBERT on SST-2\n"
                "```python\n"
                ...
                "```\n"
                "Typical val accuracy: **88-90 percent at lr=2e-5**, which is the sweet spot. lr=1e-5 underfits "
                "in 3 epochs, lr=5e-5 starts to over-fit / lose pretrained features."))

cells.append(md("### 8.6 Coding exercise 5 - Augmentation ablation on Flowers102\n"
                ...
                "    print(aug, run(aug))\n"
                "```\n"
                "Typical ranking (500 images, EfficientNetB0 frozen): full augmentation > MixUp-only > flips-only > none. The "
                "gap between full and none is usually **5-10 percentage points** at this dataset size."))

cells.append(md("### 8.7 Coding exercise 6 - LoRA on IMDb with PEFT\n"
                "```python\n"
                ...
                "```\n"
                "Compared to full fine-tuning you should see: **trainable params from ~110M to ~600k**, **training time -3-4x**, "
                "**accuracy within 0.5 percentage points**."))

cells.append(md("### 8.8 Coding exercise 7 - Domain adaptation MNIST -> USPS\n"
                "Train a feature extractor on MNIST and a classification head on the MNIST labels. "
                "Add an unsupervised MMD term computed on features from a mix of MNIST and USPS batches "
                "(use the helper in section 7). Compare USPS test accuracy with and without the MMD "
                "term; you should see a 3-8 pp improvement with MMD."))
