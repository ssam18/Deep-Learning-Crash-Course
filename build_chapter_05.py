"""Build Chapter 5 notebook: Recurrent Neural Networks and LSTM."""
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).parent))
from _nbutils import build_notebook, md, code

CHAPTER = 5
OUT = pathlib.Path(__file__).parent / f"Chapter_{CHAPTER:02d}" / f"Chapter_{CHAPTER:02d}_RNNs_and_LSTMs.ipynb"
...
cells.append(md(
    ...
    "\n"
    "This notebook implements a vanilla RNN from scratch, demonstrates the vanishing gradient "
    "problem, builds LSTM and GRU networks for IMDb sentiment analysis, and walks through "
    "tokenisation, padding, embeddings and temperature-controlled text generation.\n"
))
...
cells.append(md("## 2. Vanilla RNN forward pass from scratch\n"
                "\n"
                "$h_t = \\tanh(W_{xh} x_t + W_{hh} h_{t-1} + b_h)$. Same parameters used at every "
                "time step (parameter sharing)."))
cells.append(code(
    "class VanillaRNN:\n"
...
cells.append(md("## 3. Vanishing gradient demonstration\n"
                "\n"
                "We multiply many copies of $\\tanh'(z) \\cdot W_{hh}$ and watch the gradient norm "
...
cells.append(md("## 4. Text preprocessing: tokenise, vocab, pad\n"
                "\n"
...
cells.append(md("## 5. LSTM for IMDb sentiment classification (Keras)"))
cells.append(code(
...
cells.append(md("## 6. GRU comparison in PyTorch\n"
                "\n"
...
cells.append(md("## 7. Temperature-controlled text generation\n"
                "\n"
...
cells.append(md("### 8.2 Trace of a vanilla RNN on a 3-token sequence with hidden size 4\n"
                ...
                "$h_t$ summarises everything the network has seen so far - its content evolves "
                "as new tokens are observed."))
cells.append(code(
...
cells.append(md("### 8.3 Mathematical reason gradients vanish during BPTT\n"
                "Unrolling the RNN for $T$ steps and computing $\\partial L / \\partial h_t$ produces "
                "$\\prod_{k=t+1}^{T} \\text{diag}(1 - \\tanh^2(z_k)) W_{hh}^T$. The diagonal entries are "
                "always between 0 and 1, and the spectral radius of $W_{hh}$ is rarely exactly 1, so "
                "the product is either contracting or expanding. For random initialisations the "
                "common case is contraction, and after $\\sim 20-50$ steps the gradient has fallen "
                "below machine epsilon."))

cells.append(md("### 8.4 LSTM forget gate vs GRU update gate\n"
                "Both gates decide how much of the previous state to keep. The LSTM forget gate "
                "$f_t$ scalar-multiplies the *cell state* $c_{t-1}$ before adding the input gate "
                "term. The GRU update gate $z_t$ interpolates between the previous hidden state "
                "and the candidate state $\\tilde{h}_t$: $h_t = (1 - z_t) h_{t-1} + z_t \\tilde{h}_t$. "
                "The LSTM has separate forget and input gates - two independent decisions - "
                "while the GRU couples them into one $z_t$. The LSTM also exposes a separate "
                "*output gate* on the cell state, which the GRU lacks."))

cells.append(md("### 8.5 95% train vs 72% test on an LSTM sentiment classifier\n"
...
cells.append(md("### 8.6 End-to-end pipeline for 5,000 customer-support emails (8 classes)\n"
...
cells.append(md("### 8.7 Bi-LSTM for sentiment but not for generation\n"
                "Sentiment classification consumes the whole sequence first and then produces one label, "
                "so peeking at future tokens is fine. Generation produces one token at a time and the "
                "future has not yet been seen, so a model that depends on future context cannot be used "
                "auto-regressively. Bidirectional models are appropriate when the *full* input is "
                "available at inference time; otherwise, stick to a causal (left-to-right) model."))

cells.append(md("### 8.8 Temperature-based sampling\n"
                "Already implemented in section 7. As $T \\to 0$, the softmax becomes a one-hot at the "
                "argmax -> deterministic greedy decoding. As $T \\to \\infty$, the softmax becomes "
                "uniform over the vocabulary -> random gibberish. A value of $T \\approx 0.7$-$1.0$ is a good "
                "default for natural-text generation."))
