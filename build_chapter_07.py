"""Build Chapter 7 notebook: Generative Models (GANs, VAEs, Diffusion)."""
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).parent))
from _nbutils import build_notebook, md, code

CHAPTER = 7
OUT = pathlib.Path(__file__).parent / f"Chapter_{CHAPTER:02d}" / f"Chapter_{CHAPTER:02d}_Generative_Models.ipynb"
...
cells.append(md(
    ...
    "\n"
    "We train a small DCGAN on MNIST, build a VAE on the same dataset, walk through a "
    "DDPM-style diffusion model on synthetic 2-D data, evaluate generative quality with "
    "FID, and finish with the chapter exercises.\n"
    "\n"
    "Many cells in this chapter need a GPU for full-resolution faces. The notebook is sized "
    "to run end-to-end on CPU in a few minutes by using small models and MNIST.\n"
))
...
cells.append(md("## 2. A 1-D toy GAN\n"
                "\n"
                "Before convolutional GANs, we visualise the minimax game on a 1-D bimodal target "
                "distribution. The generator's samples should converge onto the data's two modes."))
cells.append(code(
    "try:\n"
    "    import torch\n"
...
cells.append(md("## 3. Tiny DCGAN on MNIST\n"
                "\n"
                "Stride-2 convolutions in the discriminator, transposed convolutions in the "
                "generator, BatchNorm everywhere, ReLU in G, LeakyReLU in D - the original DCGAN "
                "recipe."))
cells.append(code(
    "try:\n"
    "    import torch\n"
...
cells.append(md("## 4. Variational autoencoder on MNIST"))
cells.append(code(
    "try:\n"
    "    import torch\n"
    "    import torch.nn as nn\n"
...
cells.append(md("## 5. Denoising diffusion on a 2-D Swiss-roll dataset\n"
                "\n"
                "We learn $\\varepsilon_\\theta(x_t, t)$ on a synthetic 2-D distribution so the full "
                "forward and reverse processes can run in a notebook in seconds."))
cells.append(code(
    "try:\n"
    "    import torch\n"
    "    import torch.nn as nn\n"
...
cells.append(md("## 6. Evaluating generative models - FID intuition\n"
                "\n"
                "Full FID needs the Inception network. We illustrate the *concept* on a simple 2-D "
                "feature space - the same Gaussian-fit-and-Wasserstein-2 distance the real metric uses."))
cells.append(code(
...
cells.append(md("### 7.2 Optimal discriminator derivation\n"
                ...
                "Substituting back into the value function shows that the generator's optimum is "
                "$p_g = p_{\\text{data}}$, at which point $D^*(x) = 1/2$ everywhere - the discriminator is "
                "maximally uncertain. This is the Nash equilibrium."))

cells.append(md("### 7.3 Generator loss drops to zero with discriminator loss climbing\n"
...
cells.append(md("### 7.4 CycleGAN cycle-consistency vs Pix2Pix L1\n"
...
cells.append(md("### 7.5 DDPM ELBO and noise prediction\n"
                "Variational lower bound on $\\log p(x_0)$ decomposes into KL terms between $q(x_{t-1} | x_t, x_0)$ "
                "and $p_\\theta(x_{t-1} | x_t)$. The forward process is closed-form Gaussian, so the KL is "
                "quadratic in the means. After algebra, the bound's $t$-th term is\n"
                "$\\mathbb{E}\\left[ \\frac{\\beta_t^2}{2\\sigma_t^2 \\alpha_t (1-\\bar\\alpha_t)} || \\varepsilon - \\varepsilon_\\theta(x_t, t) ||^2 \\right]$.\n"
                "Dropping the weighting coefficient gives the simple training objective used in practice. "
                "Predicting $\\varepsilon$ rather than $x_0$ is equivalent (both are linear functions of "
                "each other through $x_t = \\sqrt{\\bar\\alpha_t} x_0 + \\sqrt{1-\\bar\\alpha_t} \\varepsilon$) but "
                "yields gradients that are uniformly informative across noise levels."))

cells.append(md("### 7.6 Latent diffusion (Stable Diffusion) trade-offs\n"
                "**Gained**: training and inference compute drop by ~$48^2 = 2304\\times$ (image 512x512 -> latent 64x64). "
                "A consumer GPU can sample 512x512 images in seconds.\n"
                "**Lost**: the VAE is itself a lossy compression - sharp text, faces and high-frequency details are "
                "more difficult to reconstruct than in pixel-space DDPM. Fine-grained errors (extra fingers, "
                "garbled text) often originate in the VAE, not the diffusion model.\n"
                "**4 channels** instead of 3: Stable Diffusion's latent uses 4 channels because the VAE was "
                "trained jointly with the diffusion model and 4 channels gave a better reconstruction-fidelity / "
                "compute trade-off in ablations."))

cells.append(md("### 7.7 Generative pipeline for 500 chest X-rays\n"
...
cells.append(md("### 7.8 FID vs Inception Score\n"
